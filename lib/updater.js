...
function npmInstallCommand() {
  ...
  const local = path.join(nodeDir, npmCmd);
  return fs.existsSync(local) ? local : npmCmd; // valt terug op PATH-npm als het niet naast node.exe staat
}

function runNpmInstall() {
  return new Promise((resolve, reject) => {
    execFile(npmInstallCommand(), ['install', '--omit=dev'], { cwd: PROJECT_ROOT, timeout: 5 * 60 * 1000 }, (err, stdout, stderr) => {
      if (err) return reject(new Error(stderr || err.message));
      resolve(stdout);
    });
  });
...
async function performUpdate() {
  ...
  try {
    ...
    const remoteVersion = await remotePackageVersion().catch(() => null);
    return {
      method: 'http',
      message: `Bijgewerkt via download van GitHub${remoteVersion ? ` naar v${remoteVersion}` : ''} (geen git nodig).${npmMessage}\n\nHerstart de server om de wijzigingen te laden.`
    };
  } finally {
